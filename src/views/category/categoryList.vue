<template>
  <div class="category-list">
    ...
    <el-table :data="tableData" class="table">
      ...
      <el-table-column label="图书数量">
        <template slot-scope="scope">
          <el-tag>{{scope.row.books.length}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope" width="300">
          <el-button size="small" type="primary" @click="handleEdit(scope.row._id)">编辑</el-button>
          <el-button size="small" type="primary" @click="handleDetails(scope.row._id)">详情</el-button>
...
export default {
  ...
  methods: {
    ...
    handleDetails (id) {
      this.$router.push({name:'categoryBook',query:{id}})
    },
    handleEdit (id) {
